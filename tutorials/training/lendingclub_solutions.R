...
col_types <- c('Numeric', 'Numeric', 'Numeric', 'Numeric', 'Numeric', 'Enum', 'String', 'Numeric', 
               ...
               'Time', 'Numeric', 'Time', 'Time', 'Numeric', 'Enum', 'Numeric')
loan_stats <- h2o.importFile(path = small_test, col.types = col_types)
...
## Task 3: Filter out all loans that are not completed, aka subset data
...
loan_stats$bad_loan <- loan_stats$loan_status %in% c("Charged Off", "Default", "Does not meet the credit policy.  Status:Charged Off")
...
loan_stats$int_rate <- h2o.gsub(pattern = "%", replacement = "", loan_stats$int_rate)
...
loan_stats$revol_util <- h2o.gsub(pattern = "%", replacement = "", loan_stats$revol_util)
...
loan_stats$verification_status <- as.factor(loan_stats$verification_status)
...
pred <- h2o.predict(gbm_model, loan_stats)
