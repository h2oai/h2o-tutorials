...
plot_ly(data = archetypes_x, x = ~Arch1, y = ~Arch2, mode = "markers", 
        text = ~paste0("House: ", id), color = ~color, colors = "Set1") %>%
...
We will build a model using the default parameters and evaluate it.  We will set the rank (`k`) of the model to 5 and the regularization strength to 2.  We will tune these parameters later on.  We will also tell the GLRM model to use Ordinal loss for our ordinal columns: `view`, `condition`, and `grade`.
...
$$
L_j(x_i y_j, A_{ij}) + \gamma_x \sum_{i=1}^m r_i(x_i) + \gamma_y \sum_{j=1}^n \hat{r}_j(y_j)
$$
